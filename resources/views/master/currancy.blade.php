            ...
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        ...
                        <div class="card-body table-responsive p-0 calaritytbl">
                            <form class="master_form" method="POST" action="{{ route('save.currency') }}">
                                {{ csrf_field() }}
...
@push('script')
    ...
    <script type="text/javascript">
            ...
            if (Length == 0 && check !== '') {
                var markup = `<tr>
                        ...
                        <td style="width: 100px">
                           ...
                           <input type="hidden" name="masters[${row}][type]"  value="currancy_master">
                        </td>
                        <td>
                           <input class="name add-row" type="text" name="masters[${row}][name]">
                        </td>
                        <td>
                           <input class="rate add-row" type="number" name="masters[${row}][rate]">
                        </td>
                     </tr>`;
        ...
        function get_currency() {
            ...
            postAjax("{{ route('get.currency', 'currancy_master') }}", {
                '_token': "{{ csrf_token() }}"
            }, function(res) {
                if (res.status == 'Success') {
                    setData(res.data);
        ...
        function setData(data) {
            ...
            if (data.length != 0) {
                $.each(data, function(i, color) {
                    html += `<tr>
                            ...
                            <td>
                               ...
                               <input type="hidden" name="masters[${i}][type]"  value="currancy_master">

                            </td>
                            <td>
                               <input class="rate add-row" type="number" name="masters[${i}][rate]" value="${parseFloat(color.rate).toFixed(2)}">
                            </td>
                         </tr>`;
                });
            } else {
                var html = `<tr>
                        ...
                        <td style="width: 100px">
                            <input type="text" name="masters[0][priority]"  value="1">
                           <input type="hidden" name="masters[0][type]"  value="currancy_master">
                        </td>
                        <td>
                           <input class="name add-row" type="text" name="masters[0][name]">
                        </td>
                        <td>
                           <input class="rate add-row" type="number" name="masters[0][rate]">
                        </td>
                     </tr>`;
            }
